...
class ItemBuilder<C extends ItemCubit<S>, S extends ItemState> extends StatelessWidget {
  ...
  bool _hasData(BuildContext context, S state) => hasData?.call(context, state) ?? state.item != null;
  ...
  C _cubitInstance(BuildContext context) {
    try {
      return context.read<C>();
    } catch (e) {
      print('There is no instance of cubit registered: $e');
      throw e;
    }
  }

  void _onInit(BuildContext context) => onInit != null ? onInit?.call(context) : _cubitInstance(context).load();
          ...
          builder: (context, state) {
            ...
            if (_isEmpty(context, state)) {
              if (_isError(context, state)) {
                return errorBuilder?.call(context, () => _onInit(context), state) ?? AbstractConfiguration.of(context)?.abstractItemErrorBuilder?.call(context, () => _onInit(context)) ?? AbstractItemErrorContainer(onInit: () => _onInit(context));
              } else {
                return noDataBuilder?.call(context, () => _onInit(context), state) ?? AbstractConfiguration.of(context)?.abstractItemNoDataBuilder?.call(context, () => _onInit(context)) ?? AbstractItemErrorContainer(onInit: () => _onInit(context));
              }
            }
